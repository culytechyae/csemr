# Production Deployment Installation Script (Simplified)
...
# Check if .env exists, backup if it does
if (Test-Path ".env") {
    ...
    if (Test-Path "env.production.template") {
        ...
        $envContent = ""
        Write-Host "WARNING: No .env file or template found. Please create .env manually." -ForegroundColor Yellow
    }
}

# Update DATABASE_URL with connection pooling (simple string replacement)
if ($envContent -and $envContent.Contains('DATABASE_URL=')) {
    if (-not $envContent.Contains('connection_limit')) {
        # Simple replacement - add connection pooling if ? exists, otherwise add ?
        if ($envContent.Contains('?')) {
            $envContent = $envContent -replace '(\?[^"]*)"', '$1&connection_limit=20&pool_timeout=20"'
        } else {
            $envContent = $envContent -replace '(DATABASE_URL="[^"]+)"', '$1?connection_limit=20&pool_timeout=20"'
        }
...
if ($envContent) {
    if (-not $envContent.Contains('NODE_ENV="production"')) {
        if ($envContent.Contains('NODE_ENV=')) {
